...
sentieon driver -r $fasta -t $nt -i $depCram --algo Haplotyper --genotype_model multinomial --emit_conf 30 --call_conf 30 --emit_mode gvcf ${i}.gvcf && rm $depCram && \
sentieon util vcfconvert ${i}.gvcf ${i}.gvcf.gz && rm ${i}.gvcf && rm $rawCram && rm $fq1 $fq2 && touch succeed.txt
